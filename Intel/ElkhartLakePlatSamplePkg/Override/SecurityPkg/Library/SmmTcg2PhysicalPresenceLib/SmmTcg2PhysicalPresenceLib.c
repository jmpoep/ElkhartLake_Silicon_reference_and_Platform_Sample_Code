...
#include <Protocol/EdkiiSmmVariable.h>  // RPPO-LKF-0022: RoyalParkOverrideContent
...
//
// ROYAL_PARK_OVERRIDE: RoyalParkOverrideBegin RPPO-LKF-0022
//
#pragma pack(1)
...
} PHYSICAL_PRESENCE_VARIABLE_CONTEXT;
...
extern TCG_NVS              *mTcgNvs;
EDKII_SMM_VARIABLE_PROTOCOL *mEdkiiSmmVariable;
//
...
VOID
EFIAPI
PhysicalPresenceSetVariableCallback (
  ...
  )
{
  ...
  PHYSICAL_PRESENCE_VARIABLE_CONTEXT   *PpVariableContext;

  PpVariableContext = (PHYSICAL_PRESENCE_VARIABLE_CONTEXT *) Context;
  ReturnCode = TCG_PP_SUBMIT_REQUEST_TO_PREOS_SUCCESS;

  if (EFI_ERROR (Status)) {
      ReturnCode = TCG_PP_SUBMIT_REQUEST_TO_PREOS_GENERAL_FAILURE;
      mTcgNvs->PhysicalPresence.ReturnCode = ReturnCode;
      DEBUG ((EFI_D_ERROR, "[TPM2] Set PP variable failure! Status = %r\n", Status));
      goto Exit;
  }

  if (PpVariableContext == NULL) {
    ReturnCode = TCG_PP_SUBMIT_REQUEST_TO_PREOS_GENERAL_FAILURE;
    mTcgNvs->PhysicalPresence.ReturnCode = ReturnCode;
    DEBUG ((EFI_D_ERROR, "[TPM2] Set PP variable invalid variable context!\n"));
    goto Exit;
  }

  if (*(PpVariableContext->OperationRequest) >= TCG2_PHYSICAL_PRESENCE_VENDOR_SPECIFIC_OPERATION) {
    ...
    ReturnCode = Tcg2PpVendorLibSubmitRequestToPreOSFunction (
                   *(PpVariableContext->OperationRequest),
                   Flags.PPFlags,
                   *(PpVariableContext->RequestParameter)
                   );
  }
...
Exit:
  ...
  if ((ReturnCode != TCG_PP_SUBMIT_REQUEST_TO_PREOS_SUCCESS)) {
    ...
    if (PpVariableContext != NULL) {
      *(PpVariableContext->OperationRequest) = (UINT32) PpData.PPRequest;
      *(PpVariableContext->RequestParameter) = PpData.PPRequestParameter;
    }
  }

  mTcgNvs->SmiComplete = 1;
}
  ...
  )
{
  ...
  UINTN                                      DataSize;
  EFI_TCG2_PHYSICAL_PRESENCE                 PpData;
  STATIC PHYSICAL_PRESENCE_VARIABLE_CONTEXT  PpVariableContext;  // RPPO-LKF-0022: RoyalParkOverrideContent

  PpVariableContext.OperationRequest = OperationRequest; // RPPO-LKF-0022: RoyalParkOverrideContent
  PpVariableContext.RequestParameter = RequestParameter; // RPPO-LKF-0022: RoyalParkOverrideContent
  ...
  if ((PpData.PPRequest != *OperationRequest) ||
    ...
    Status = mEdkiiSmmVariable->SmmSetVariable (               // RPPO-LKF-0022: RoyalParkOverrideContent
                                  (VOID *) &PpVariableContext, // RPPO-LKF-0022: RoyalParkOverrideContent
                                  TCG2_PHYSICAL_PRESENCE_VARIABLE,
                                  &gEfiTcg2PhysicalPresenceGuid,
                                  EFI_VARIABLE_NON_VOLATILE | EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS,
                                  DataSize,
                                  &PpData,
                                  PhysicalPresenceSetVariableCallback  // RPPO-LKF-0022: RoyalParkOverrideContent
                                  );
    if (EFI_ERROR (Status)) {
      DEBUG ((EFI_D_ERROR, "[TPM2] Set PP variable failure! Status = %r\n", Status));
      ReturnCode = TCG_PP_SUBMIT_REQUEST_TO_PREOS_GENERAL_FAILURE;
    ...
    PhysicalPresenceSetVariableCallback ((VOID *) &PpVariableContext, EFI_SUCCESS);
  }
...
EXIT:
  ...
  if (ReturnCode != TCG_PP_SUBMIT_REQUEST_TO_PREOS_SUCCESS) {
    ...
    *RequestParameter = PpData.PPRequestParameter;
    mTcgNvs->SmiComplete = 1; // RPPO-LKF-0022: RoyalParkOverrideContent
  }
  ...
  )
{
  ...
  // ROYAL_PARK_OVERRIDE: RoyalParkOverrideBegin RPPO-LKF-0022
  //
  Status = gSmst->SmmLocateProtocol (&gEdkiiSmmVariableProtocolGuid, NULL, (VOID**) &mEdkiiSmmVariable);
  ASSERT_EFI_ERROR (Status);
  //
  // ROYAL_PARK_OVERRIDE: RoyalParkOverrideEnd
  //
