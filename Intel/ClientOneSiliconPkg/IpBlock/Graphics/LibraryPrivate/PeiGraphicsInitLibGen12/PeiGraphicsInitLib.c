  ...
  )
{
  ...
  if (StallCount < GT_WAIT_TIMEOUT) {
    ...
    DEBUG ((DEBUG_INFO, " GT Readiness timeout\n"));
    ASSERT ((StallCount < GT_WAIT_TIMEOUT));  // @todo need to address, always timeout for ICL.
    return EFI_TIMEOUT;
  }
}
  ...
  )
{
  ...
  PcmSize = (PAVP_PCM_SIZE_2_MB * 0x100000);

  DfdBase = (UINT32) (GMSBase + GMSSize * 0x100000 - PcmSize - DFD_RESTORE_SIZE);
  DfdLimit = (UINT32) (DfdBase + DFD_RESTORE_SIZE - 1);

  ///
  /// Programming Doorbell Context base low.
  ///
  DEBUG ((DEBUG_INFO, "DoorbellCtxBase: 0x%x\n", DoorbellCtxBase));
  ///
  /// 0xDC8 [31:6] = Base[31:6], 0xDC8[0] = 1
  ///
  Data32 = (DoorbellCtxBase & 0xFFFFFFC0)| BIT0;
  MmioWrite32 (GttMmAdr + 0xDC8, Data32);
  MmioWrite32 (GttMmAdr + 0xDCC, 0);

  if (GtPreMemConfig->DfdRestoreEnable) {
    ///
    /// Programming DFD Limit
    ///
    DEBUG ((DEBUG_INFO, "DfdLimit: 0x%x\n", DfdLimit));
    MmioWrite32 (GttMmAdr + R_SA_GTTMMADR_DFD_LIMIT_OFFSET, DfdLimit);
    ///
    /// Programming DFD Base
    ///
    DEBUG ((DEBUG_INFO, "DfdBase: 0x%x\n", DfdBase));
    MmioWrite32 (GttMmAdr + R_SA_GTTMMADR_DFD_BASE_OFFSET, DfdBase);
  }

  ///
  /// Programming Context Base
  ///
