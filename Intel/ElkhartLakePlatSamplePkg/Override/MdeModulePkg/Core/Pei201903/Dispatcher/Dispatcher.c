...
//
// FSPO-0008: RoyalParkOverrideBegin
//
/**
...
//
// FSPO-0008: RoyalParkOverrideEnd
//

/**
  Migrate FVs out of Temporary Ram before cache gets flushed.
...
//
// RPPO-LKF-0031: OverrideBegin
//
  // Migrate Notify Pointers of PEI_CORE from temporary memory to newly
  // loaded PEI_CORE in permanent memory.
  ConvertPeiCorePpiPointers (Private, Private->Fv[0]);
//
// RPPO-LKF-0031: OverrideEnd
//
  ...
  for (FvIndex = 0; FvIndex < Private->FvCount; FvIndex++) {
    ...
    if (!(((EFI_PHYSICAL_ADDRESS) FvHeader >= Private->PhysicalMemoryBegin) &&
      ...
      if (Private->Fv[FvIndex].ScanFv) {
        for (FileIndex = 0; (FileIndex < Private->Fv[FvIndex].PeimCount) && (Private->Fv[FvIndex].FvFileHandles[FileIndex] != NULL); FileIndex++) {
  ...
  )
{
  ...
  if ((Private->PeiMemoryInstalled) && (Private->HobList.HandoffInformationTable->BootMode != BOOT_ON_S3_RESUME || PcdGetBool (PcdShadowPeimOnS3Boot))) {
    ...
    for (Index1 = 0; Index1 < Private->FvCount; Index1++) {
      for (Index2 = 0; (Index2 < Private->Fv[Index1].PeimCount) && (Private->Fv[Index1].FvFileHandles[Index2] != NULL); Index2++) {
  ...
  do {
    ...
    for (FvCount = Private->CurrentPeimFvCount; FvCount < Private->FvCount; FvCount++) {
      ...
      for (PeimCount = Private->CurrentPeimCount;
           (PeimCount < Private->Fv[FvCount].PeimCount) && (Private->CurrentFvFileHandles[PeimCount] != NULL);
           PeimCount++) {
