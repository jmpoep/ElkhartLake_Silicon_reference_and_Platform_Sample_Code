/** @file
  Produce FMP instances for SIIP FW Sub Region
...
#define SIIP_FW_FMP_PRIVATE_DATA_SIGNATURE      SIGNATURE_32('P', 'S', 'E', 'F')
#define SIIP_FW_FV_IMAGE_ID                     SIGNATURE_64('P', 'S', 'E','_','F','W',' ',' ')
#define SIIP_FW_FMP_LAST_ATTEMPT_VARIABLE_NAME  L"SiipFwLastAttempVar"

FLASH_SUB_REGION_INFO mSiipFwSpiSubRegion = {
  &gFmpCapsuleSiipFwFileGuid,
  FixedPcdGet32 (PcdFlashFvSiipFwOffset),
  ...
  )
{
  ...
  *Size = FixedPcdGet32 (PcdFlashFvSiipFwSize);
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  //
  if (*ImageSize < PcdGet32 (PcdFlashFvSiipFwSize)) {
    *ImageSize = PcdGet32 (PcdFlashFvSiipFwSize);
  ...
  *ImageSize = PcdGet32 (PcdFlashFvSiipFwSize);
  CopyMem (
    Image,
    (VOID *)(UINTN) ((PcdGet32 (PcdFlashFvSiipFwOffset) + PcdGet32 (PcdBiosAreaBaseAddress)) & (UINT64)~B_FMP_HIGH_32BIT_MASK),
    *ImageSize
    );
  ...
  )
{
  ...
  Status = SubRegionWrite ( (VOID *) Image,
                          ImageSize,
                          SIIP_FW_FMP_PRIVATE_DATA_SIGNATURE,
                          &mSiipFwSpiSubRegion,
                          AbortReason
                          );
  ...
  )
{
  ...
  *ImageUpdateable = IMAGE_UPDATABLE_VALID;
  ...
  /*
  if (!IsImageForSubRegion ((VOID *) Image, ImageSize, SIIP_FW_SUB_REGION_TYPE)) {
    *ImageUpdateable = IMAGE_UPDATABLE_INVALID;
  }
  */
...
{
  DEBUG ((DEBUG_INFO, "FmpDeviceLib(SIIP_FW): FmpDeviceLock() for system FLASH\n"));
