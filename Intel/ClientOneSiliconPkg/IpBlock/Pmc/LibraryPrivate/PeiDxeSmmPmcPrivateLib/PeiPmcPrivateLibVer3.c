  ...
  )
{
  ...
  MmioWrite32 (PwrmBase + R_EHL_PCH_PMC_PWRM_LPM_EN, LpmS0ixSubStateEnable.Val);
  ...
  //
  MmioWrite32 (PwrmBase + R_EHL_PCH_PMC_PWRM_LPM_PRI, 0x01346257);

  MmioWrite32 (PwrmBase + R_EHL_PCH_PMC_PWRM_LPM_STS_LATCH_EN, 0xFF);
}
  ...
  )
{
  ...
  /// MODPHY_PM_CFG3 (0x10C8[1]) = 1, ModPHY Per-Lane SUS Well Dynamic Gating Enable
  /// MODPHY_PM_CFG3 (0x10C8[0]) = 1, C10 qualification for MPHY power gating Enable
  ///
  MmioOr32 (
    PmcGetPwrmBase () + R_PMC_PWRM_MODPHY_PM_CFG3,
    B_PMC_PWRM_MODPHY_PM_CFG3_MLSPDDGE | B_PMC_PWRM_MODPHY_PM_CFG3_MPLSPDDGE | B_PMC_PWRM_MODPHY_PM_CFG3_C10_QUAL_MPHYPG
    );
}
  ...
  )
{
  ...
  ///
  /// PWRMBASE + 0x1BD0 [31,27,15] = 1b, 1b, 1b, 1b, 1b
  /// PWRMBASE + 0x1BD0 [28] = 1b
  /// PWRMBASE + 0x1BD0 [21:16] = 0Ah
  /// PWRMBASE + 0x1BD0 [30] = 1b, done in a separate programming
  ///
  Data32And = (UINT32) ~(BIT28 | (0x3F << 16));
  Data32Or = BIT31 | BIT27 | (0x0A << 16) | BIT15;
  Data32Or |= BIT28;
  MmioAndThenOr32 (
