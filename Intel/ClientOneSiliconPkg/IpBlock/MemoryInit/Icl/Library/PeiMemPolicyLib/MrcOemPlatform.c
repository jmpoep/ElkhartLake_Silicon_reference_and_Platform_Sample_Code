/** @file
  Sample implementation of SA_FUNCTION_CALLS member functions that are used by MRC,
  ...
  )
{
  ...
  if (RETURN_SUCCESS == Status) {
    switch (Buffer[SPD_DDR3_SDRAM_TYPE_OFFSET]) {
      case SPD_DDR4_SDRAM_TYPE_NUMBER:
      default:
        TableSelect = (MrcSpdOffsetTable *) SpdDdr4Table;
        Stop = (SpdDdr4TableSize / sizeof (MrcSpdOffsetTable));
        break;
      case SPD_LPDDR3_SDRAM_TYPE_NUMBER:
      case SPD_LPDDR4_SDRAM_TYPE_NUMBER:
      case SPD_LPDDR4X_SDRAM_TYPE_NUMBER:
        TableSelect = (MrcSpdOffsetTable *) SpdLpddrTable;
        Stop = (SpdLpddrTableSize / sizeof (MrcSpdOffsetTable));
        break;
    }
    if (TableSelect == NULL) {
      return FALSE;
  ...
  )
{
  ...
  Start  = SaGetCpuTimeNanoSec (GlobalData);
  Finish = Start + DelayNs;
  Done   = FALSE;


  do {
    Now = SaGetCpuTimeNanoSec (GlobalData);
    if (Finish > Start) {
      if (Now >= Finish) {
        Done = TRUE;
      }
