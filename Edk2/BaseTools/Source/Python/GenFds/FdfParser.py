...
from Common.Misc import PathClass, tdict, ProcessDuplicatedInf # CCEO-0014: RoyalParkOverrideContent
...
#
# CCEO-0014: RoyalParkOverrideBegin
#
...
#
# CCEO-0014: RoyalParkOverrideEnd
#
            FileLineTuple = GetRealFileLine(self.FileName, self.CurrentLineNumber)
    ...
    def _GetRuleFileStatements(self):
        ...
        if not self._IsKeyword("$(NAMED_GUID)"):
            if not self._GetNextWord():
                # CCEO-0021: RoyalParkOverrideContentBegin
                ...
                # CCEO-0021: RoyalParkOverrideContentEnd
            if self._Token == 'PCD':
                if not self._IsToken("("):
                    raise Warning.Expected("'('", self.FileName, self.CurrentLineNumber)
                PcdPair = self._GetNextPcdSettings()
    ...
    def _GetEfiSection(self, Obj):
        OldPos = self.GetFileBufferPos()
        EfiSectionObj = EfiSection() # CCEO-0021: RoyalParkOverrideContent
        if not self._GetNextWord():
            # CCEO-0021: RoyalParkOverrideContentBegin
