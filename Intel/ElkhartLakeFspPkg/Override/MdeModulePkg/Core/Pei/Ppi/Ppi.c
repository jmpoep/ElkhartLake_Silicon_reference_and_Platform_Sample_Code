  ...
  )
{
  ...
  for (Index = 0; Index < PcdGet32 (PcdPeiCoreMaxPpiSupported); Index++) {
    if ( (INTN) Index < PrivateData->PpiData.PpiListEnd || (INTN) Index > PrivateData->PpiData.NotifyListEnd) {
      ConvertSinglePpiPointer (
        SecCoreData,
        PrivateData,
  ...
  )
{
  ...
  if (!EFI_ERROR (Status) && (PeiCoreFvLocationPpi->PeiCoreFvLocation != 0)) {
    PeiCoreFv = TRUE;
  }

  for (Index = 0; Index < PrivateData->FvCount; Index ++) {
    if (PeiCoreFv == TRUE) {
      if ((UINT32) PrivateData->Fv[Index].FvHandle == PeiCoreFvLocationPpi->PeiCoreFvLocation) {
        ...
        if (!EFI_ERROR (Status)) {
          CoreFvHandle = PrivateData->Fv[Index];
          break;
        }
      }
    } else {
