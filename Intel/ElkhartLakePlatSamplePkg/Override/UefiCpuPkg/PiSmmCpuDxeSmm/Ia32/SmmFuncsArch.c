  ...
  )
{
  ...
  UINTN                     InterruptShadowStack; // CCEO-0016: RoyalParkOverrideContent
  ...
  if (FeaturePcdGet (PcdCpuSmmStackGuard)) {
    ...
    GdtTssTableSize = (gcSmiGdtr.Limit + 1 + TSS_SIZE + EXCEPTION_TSS_SIZE + 7) & ~7; // 8 bytes aligned // CCEO-0016: RoyalParkOverrideContent
    ...
    for (Index = 0; Index < gSmmCpuPrivate->SmmCoreEntryContext.NumberOfCpus; Index++) {
      CopyMem (GdtTssTables + GdtTableStepSize * Index, (VOID*)(UINTN)gcSmiGdtr.Base, gcSmiGdtr.Limit + 1 + TSS_SIZE + EXCEPTION_TSS_SIZE); // CCEO-0016: RoyalParkOverrideContent
      //
