  ...
  )
{
  ...
  if (RpIndex < 4) {
    Fid = (UINT16) ((RpDevice << 3) | (GetControllerRpIndex (RpIndex)));
  } else {
    Fid = 0;
  }
  ...
  )
{
  ...
  if (IsRpMultiVC (RpIndex)) {
    ASSERT ((GetControllerRpIndex (RpIndex) + RpLane) < 2);
  } else {
    ASSERT ((GetControllerRpIndex (RpIndex) + RpLane) < 4);
  }

  ControllerIndex = GetControllerIndex (RpIndex);
  ControllerLane  = GetControllerRpIndex (RpIndex) + RpLane;
  if (IsPcieLaneReversalEnabled (RpIndex)) {
    if (IsRpMultiVC (RpIndex)) {
      ControllerLane = 1 - ControllerLane;
    } else {
      ControllerLane = 3 - ControllerLane;
    }
  }
  if (IsCtrlMultiVC (ControllerIndex)) {
    return (4 + ((ControllerIndex - 1) * 2) + ControllerLane);
  } else {
...
{
  return (UINT8) ((PciSegmentRead16 (RpBase + R_PCH_PCIE_CFG_LSTS) & B_PCIE_LSTS_NLW) >> N_PCIE_LSTS_NLW);
}
  ...
  )
{
  UINT32       Data32;
  EFI_STATUS   Status;
  Status = PchSbiRpPciRead32 (PchGetPcieFirstPortIndex (RpIndex), R_PCH_PCIE_CFG_PCIEDBG, &Data32);
  if (Status != EFI_SUCCESS) {
    return 0;
  } else {
  ...
  )
{
  ...
  ControllerIndex = GetControllerIndex (RpIndex);
  return GetControllerFirstRpIndex (ControllerIndex);
}
  ...
  )
{
  ...
  ///
  if (IsCpuPcie) {
    Ctrl1Register |= (0xFA << N_PCIE_EX_L1SCTL1_L12LTRTLV);
    Ctrl1Register |= (2 << N_PCIE_EX_L1SCTL1_L12LTRTLSV);
    Ctrl2Register |= (2);
    Ctrl2Register |= (4 << N_PCIE_EX_L1SCTL2_POWT);
  } else {
...
/**
  ...
  @param[in] Segment,Bus,Device,Function    address of currently visited PCIe device
  @param[in] PcieRpLtrConfig              PCIe LTR Override values
  ...
  )
{
  ...
  if (PcieRpLtrConfig->ForceLtrOverride || TreeLtr.ForceOverride) {
    OvrEn |= B_PCH_PCIE_CFG_LTROVR2_FORCE_OVERRIDE;
  }
  if (PcieRpLtrConfig->LtrConfigLock == TRUE) {
    OvrEn |= B_PCH_PCIE_CFG_LTROVR2_LOCK;
    if (IsCpuPcie) {
      LtrCfgLock |= B_PCIE_LPCR_LTRCFGLOCK;
    }
  }

  if (PcieRpLtrConfig->SnoopLatencyOverrideMode == 1) {
    OvrEn |= B_PCH_PCIE_CFG_LTROVR2_LTRSOVREN;
    OvrVal |= (UINT32) (PcieRpLtrConfig->SnoopLatencyOverrideValue);
    OvrVal |= (UINT32) (PcieRpLtrConfig->SnoopLatencyOverrideMultiplier << 10);
    OvrVal |= B_PCH_PCIE_CFG_LTROVR_LTRSROVR;
  } else if (PcieRpLtrConfig->SnoopLatencyOverrideMode == 2) {
    if (TreeLtr.MaxSnoopLatencyRequirement) {
      OvrEn |= B_PCH_PCIE_CFG_LTROVR2_LTRSOVREN;
      OvrVal |= TreeLtr.MaxSnoopLatencyValue;
      OvrVal |= (UINT32) (TreeLtr.MaxSnoopLatencyScale << 10);
      OvrVal |= B_PCH_PCIE_CFG_LTROVR_LTRSROVR;
    }
  }
  if (PcieRpLtrConfig->NonSnoopLatencyOverrideMode == 1) {
    OvrEn |= B_PCH_PCIE_CFG_LTROVR2_LTRNSOVREN;
    OvrVal |= (UINT32) (PcieRpLtrConfig->NonSnoopLatencyOverrideValue << 16);
    OvrVal |= (UINT32) (PcieRpLtrConfig->NonSnoopLatencyOverrideMultiplier << 26);
    OvrVal |= B_PCH_PCIE_CFG_LTROVR_LTRNSROVR;
  } else if (PcieRpLtrConfig->NonSnoopLatencyOverrideMode == 2) {
    if (TreeLtr.MaxNoSnoopLatencyRequirement) {
      OvrEn |= B_PCH_PCIE_CFG_LTROVR2_LTRNSOVREN;
      OvrVal |= (UINT32) (TreeLtr.MaxNoSnoopLatencyValue << 16);
      OvrVal |= (UINT32) (TreeLtr.MaxNoSnoopLatencyScale << 26);
      OvrVal |= B_PCH_PCIE_CFG_LTROVR_LTRNSROVR;
    }
  }
  ...
  if (IsCpuPcie) {
    PciSegmentOr32(RpBase + R_PCIE_LPCR, LtrCfgLock);
  }
  DEBUG ((DEBUG_INFO, "ConfigureRpLtrOverride %x:%x Val %x En %x LtrCfgLock %x\n", RpSbdf.Dev, RpSbdf.Func, OvrVal, OvrEn, LtrCfgLock));
}
  ...
  )
{
  ...
  if ((L1LowSupported || L1ssEnabled) && !IsCpuPcie) {
    ///
    /// f.  Set Dxx:Fn:420h[0] to 1b prior to L1 enabling if any L1substate is enabled (including L1LOW)
    ///
    PciSegmentOr32 (RpBase + R_PCH_PCIE_CFG_PCIEPMECTL,(UINT32) B_PCH_PCIE_CFG_PCIEPMECTL_L1FSOE);
  } else {
    PciSegmentAnd32 (RpBase + R_PCH_PCIE_CFG_PCIEPMECTL,(UINT32) ~B_PCH_PCIE_CFG_PCIEPMECTL_L1FSOE);
  }
}
  ...
  )
{
  ...
  PcieBase  = PCI_SEGMENT_LIB_ADDRESS (0, Sbfd.Bus, Sbfd.Dev, Sbfd.Func, 0);

  DEBUG ((DEBUG_INFO, "Programming Multi Virtual Channel %x configuration for BDF %x|%x|%x\n", ExtVc.ExtVcCount, Sbfd.Bus, Sbfd.Dev, Sbfd.Func));
  DEBUG ((DEBUG_INFO, "Vc0Ctl Reg 0x%4X, Vc1Ctl Reg 0x%4X\n", Vc0CtlReg, Vc1CtlReg));
  //
  // Program VC0CTL.TVM [7:1] to 0011111b
  //
  Data32And = (UINT32) ~B_PCIE_MVC_V0CTL_TVM;
  Data32Or = (UINT32) (V_PCIE_MVC_V0CTL_TVM_EN_VC << N_PCIE_MVC_V0CTL_TVM);
  ...
  //
  Data32And = (UINT32)~(B_PCIE_MVC_V1CTL_TVM | B_PCIE_MVC_V1CTL_ID_MASK | B_PCIE_MVC_V1CTL_EN);
  Data32Or = (UINT32)((V_PCIE_MVC_V1CTL_TVM_EN_VC << N_PCIE_MVC_V1CTL_TVM)
                      | (V_PCIE_MVC_V1CTL_ID_ONE << N_PCIE_MVC_V1CTL_ID)
                      | B_PCIE_MVC_V1CTL_EN);
  ...
  )
{
  ...
  PcieBase = PCI_SEGMENT_LIB_ADDRESS (0, Sbdf.Bus, Sbdf.Dev, Sbdf.Func, 0);
  MyExtVc.ExtVcCount = 0;
  MyExtVc.CapOffset = PcieBaseFindExtendedCapId (PcieBase, VC_CAP_ID);
  DEBUG ((DEBUG_INFO, "MyExtVc.CapOffset %08X\n", MyExtVc.CapOffset));

  if (MyExtVc.CapOffset) {
    MyExtVc.ExtVcCount = (PciSegmentRead8 (PcieBase + MyExtVc.CapOffset + VC_CAP_REG) & B_PCIE_MVC_PVCCR1_EVCC_MASK);
  ...
  )
{
  ...
  PolicyLtr.MaxNoSnoopLatencyScale = (PcieRpLtrConfig->LtrMaxNoSnoopLatency & 0x1c00) >> 10;
  PolicyLtr.MaxNoSnoopLatencyValue = PcieRpLtrConfig->LtrMaxNoSnoopLatency & 0x3FF;
  PolicyLtr.MaxSnoopLatencyScale   = (PcieRpLtrConfig->LtrMaxSnoopLatency & 0x1c00) >> 10;
  PolicyLtr.MaxSnoopLatencyValue   = PcieRpLtrConfig->LtrMaxSnoopLatency & 0x3FF;
  ...
  ConfigureRpLtrOverride (RpSbdf, PcieRpLtrConfig, &PmOverrideTable);
