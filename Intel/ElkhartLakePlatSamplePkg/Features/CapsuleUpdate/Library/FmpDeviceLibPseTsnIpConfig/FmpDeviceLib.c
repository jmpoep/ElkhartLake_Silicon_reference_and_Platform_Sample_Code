/** @file
  Produce FMP instances for Pse Tsn Ip Config Sub Region
...
#define PSE_TSN_IP_CONFIG_FMP_PRIVATE_DATA_SIGNATURE      SIGNATURE_32('T', 'I', 'P', 'C')
#define PSE_TSN_IP_CONFIG_FV_IMAGE_ID                     SIGNATURE_64('T', 'S', 'N','I','P','C','O','N')
#define PSE_TSN_IP_CONFIG_FMP_LAST_ATTEMPT_VARIABLE_NAME  L"PseTsnIpConfigLastAttempVar"

FLASH_SUB_REGION_INFO mPseTsnIpConfigSpiSubRegion = {
  &gFmpCapsulePseTsnIpConfigFileGuid,
  FixedPcdGet32 (PcdFlashFvPseTsnIpConfigOffset),
  ...
  )
{
  ...
  *Size = FixedPcdGet32 (PcdFlashFvPseTsnIpConfigSize);
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  //
  if (*ImageSize < PcdGet32 (PcdFlashFvPseTsnIpConfigSize)) {
    *ImageSize = PcdGet32 (PcdFlashFvPseTsnIpConfigSize);
  ...
  *ImageSize = PcdGet32 (PcdFlashFvPseTsnIpConfigSize);
  CopyMem (
    Image,
    (VOID *)(UINTN) ((PcdGet32 (PcdFlashFvPseTsnIpConfigOffset) + PcdGet32 (PcdBiosAreaBaseAddress)) & (UINT64)~B_FMP_HIGH_32BIT_MASK),
    *ImageSize
    );
  ...
  )
{
  ...
  Status = SubRegionWrite ( (VOID *) Image,
                          ImageSize,
                          PSE_TSN_IP_CONFIG_FMP_PRIVATE_DATA_SIGNATURE,
                          &mPseTsnIpConfigSpiSubRegion,
                          AbortReason
                          );
  ...
  )
{
  ...
  *ImageUpdateable = IMAGE_UPDATABLE_VALID;
  ...
  //
  // Make sure the image has the information needed for updating Sub-Region
  //
  // Add back when sub-region sanity check is ready
  /*
  if (!IsImageForSubRegion ((VOID *) Image, ImageSize, PSE_TSN_IP_CONFIG_SUB_REGION_TYPE)) {
    *ImageUpdateable = IMAGE_UPDATABLE_INVALID;
  }
  */
...
{
  DEBUG ((DEBUG_INFO, "FmpDeviceLib(PseTsnIpConfig): FmpDeviceLock() for system FLASH\n"));
...
EFI_STATUS
EFIAPI
FmpDeviceLibPseTsnIpConfigConstructor (
