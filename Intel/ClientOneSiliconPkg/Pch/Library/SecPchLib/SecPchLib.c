...
VOID
EFIAPI
SerialIoUartDebugConfiguration (
  VOID
  )
{
  ...
  UINT32                 SerialIoUartPciMmioBase;
  CHAR8                  CarInitBuffer[32];

#if FixedPcdGetBool(PcdFspBinaryEnable) == 1
  AsciiSPrint (CarInitBuffer, sizeof (CarInitBuffer), "FSP-T: CAR Init\n");
  SerialIoUartDebugConfigurationWrapper (&UartDeviceConfig, &SerialIoUartDebugEnable, &SerialIoUartNumber, &SerialIoUartPciMmioBase);
#else
  ...
  if (SerialIoUartDebugEnable == 1) {
    if (UartDeviceConfig.Mode == SerialIoUartPci) {
      SerialIoUartSetMmioInPciMode (SerialIoUartNumber, (UINT64)SerialIoUartPciMmioBase);
    }
    SerialIoUartConfiguration (SerialIoUartNumber, &UartDeviceConfig, TRUE, TRUE);
    SerialIoUartWrite (SerialIoUartNumber, (UINT8 *)CarInitBuffer, AsciiStrLen (CarInitBuffer));
  }
}

/**
  This is helper function to initialize SerialIoSpiController in early init for getting SPI Postcodes.
**/
VOID
EFIAPI
SerialIoSpiPostCodeConfiguration (
  VOID
  )
{
  SERIAL_IO_SPI_CONFIG  SpiDeviceConfig;

  //
  // Initialize the SPI Controller as Hidden Mode
  //
  SpiDeviceConfig.Mode = SerialIoSpiHidden;
  SpiDeviceConfig.DefaultCsOutput = 0;
  SpiDeviceConfig.CsPolarity[0] = 1;
  SpiDeviceConfig.CsPolarity[1] = 1;
  SpiDeviceConfig.CsEnable[0] = 1;
  SpiDeviceConfig.CsEnable[1] = 0;
  SpiDeviceConfig.MisoEnable = 0;
  SerialIoSpiConfiguration(SERIAL_IO_SPI_POSTCODE_NUMBER, &SpiDeviceConfig, TRUE, TRUE);
}
