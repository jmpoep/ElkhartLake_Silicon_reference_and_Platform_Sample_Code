...
/**
  This function performs initial DFx (Jtag) related configurations
**/
VOID
GpioConfigureDFx (
  VOID
  )
{
  ...
  //
  NumOfPads = GpioGetDFxStandbyStateMaskedPadsList (&PadsList);

  for (Index = 0; Index < NumOfPads; Index++) {
    GpioWritePadCfgReg (
      PadsList[Index],
      1,
      (UINT32)~B_GPIO_PCR_STANDBY_STATE,
      V_GPIO_PCR_STANDBY_STATE_MASKED << N_GPIO_PCR_STANDBY_STATE
    );
  }
}
