...
if not defined PLATFORM_BOARD_PACKAGE set PLATFORM_BOARD_PACKAGE=ElkhartLakeOpenBoardPkg
if not defined BOARD set BOARD=ElkhartLakeSimics
...
@if %FSP_WRAPPER_BUILD% EQU TRUE (
  ...
  echo "" > %WORKSPACE_FSP_BIN%\ElkhartLakeFspBinPkg\Fsp_Rebased_S_padded.fd
  attrib -r %WORKSPACE_FSP_BIN%\ElkhartLakeFspBinPkg\Fsp_Rebased_S_padded.fd
)
