...
2. Under ElkhartLakePlatSamplePkg, open Command prompt, run:
     capsulebuild [target:all/bios/ec/me][-mever ^<ME Build Version^>][-pdtver ^<PDT Version^>][help]

        target   Assign Capsule build targets: all/bios/me/ec/pdt, default is building Monolithic Capsule
...
3. If the build is success, IclSystemFwXXXX.cap would be observed in CapsuleOutput directory.
4. Windows Update driver (if generated) would be found in CapsuleOutput\WindowsCapsule

Examples:
   a. capsulebuild.bat
        => Default build. Build a Monolithic capsule. IclSystemFwMono_xxxx.cap would be observed in CapsuleOutput directory.
           Windows update driver (if generated) would be found in CapsuleOutput\WindowsCapsule\Monolithic

   b. capsulebuild.bat ec
        => Build a standalone ec capsule. IclSystemFwEc_xxxx.cap would be observed in CapsuleOutput directory.
           Windows update driver (if generated) would be found in CapsuleOutput\WindowsCapsule\Ec

   c. capsulebuild.bat bios me -mever 1033
        => Build both standalone ME capsule and BIOS capsule. 1033 indicates ME build verion of Me.bin
           IclSystemFwMe_1033.cap and IclSystemFwBios_xxxx.cap would be observed in CapsuleOutput directory.
   ...
   d. capsulebuild.bat all -mever 1033
        => Build Monolithic Capsule and standalone BIOS/ME/EC Capsules. 1033 indicates ME build verion of Me.bin
           IclSystemFwMono_xxxx.cap, IclSystemFwBios_xxxx.cap, IclSystemFwMe_1033.cap and IclSystemFwEc_xxxx.cap would be observed in CapsuleOutput directory.
...
3. fs0:> CapsuleApp <Capsule file name (e.g. IclSystemFwXXXX.Cap)>
...
Installing the Firmware driver package:
        1. Copy IclSystemFwXXXX directory to SUT
        ...
        3. Enter to driver package created. Run "pnputil -i -a IclSystemFwXXXX.inf" as Administrator. Then restart system.