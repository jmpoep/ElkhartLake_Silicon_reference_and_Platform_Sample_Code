...
/// tCAENT = 250ns
/// tCAENT is same for both LP4 and LP5
...
#define IS_MC_SUB_CH(Lpddr, Channel) ((Lpddr) && ((Channel) % 2))
#define LP_IP_CH(Lpddr, Channel) ((Lpddr) ? (Channel) / 2 : (Channel))
...
#define PatCADBCol                          7
#define PatCADBRow                          8
#define PatWrEndless                        9
#define PatRdEndless                        10
#define PatWrRdEndless                      11
...
/**
 1D Margin Types:
  ...
  WrTLp5:   Shifts Whole Write DQ range
...
typedef enum {
  ...
  OptCmdDSDnCoarse,
  OptCmdDSUpCoarse,
  OptCmdTxEqCmdDS, //param0=CmdTxEq param1=CmdDS
  OptCCCSComp,
  OptVccDLLBypass,
  OptRxVrefVttDecap,
  OptRxVrefVddqDecap,
...
typedef struct {
  MRC_PAT_SRC_TYPE PatSource;       ///< Defines the source of patterns: Static, Dynamic
  UINT16 IncScale;
...
extern MRC_BG_BANK_PAIR Ddr4x8BankMapDg[MAX_DDR4_x8_BANKS];
extern MRC_BG_BANK_PAIR Ddr4x16BankMapDg[MAX_DDR4_x16_BANKS];
extern MRC_BG_BANK_PAIR Ddr4BankMapSg[MAX_DDR4_x16_BANKS / 2];
///
...
/**
  Set default CPU Read Vref for DDR4 and LPDDR
...
/**
  ...
  @param[in] Controller - Memory Controller to use.
...
/**
  Programs all the key registers to define a CPGC test as per Outputs->McChBitMask.
...
/**
  This function sets up a test with CADB for the given mask and per Outputs->McChBitMask.
  McChBitMask is initialized in MrcPretraining based on population, and used throughout internal CPGC structure.
  Modify McChBitMask to specify which MC/CH to program.

  @param[in,out] MrcDat  a   - Pointer to MRC global data.
  @param[in]     McChBitMask - Memory Controller Channel Bit mask for which test should be setup for.
...
/**
  This function sets up a MPR test for the given mask and per Outputs->McChBitMask.
  McChBitMask is initialized in MrcPretraining based on population, and used throughout internal CPGC structure.
  Modify McChBitMask to specify which MC/CH to program.

  @param[in,out] MrcData     - Pointer to MRC global data.
  @param[in]     McChBitMask - Memory Controller Channel Bit Mask to enable for the test.
  @param[in]     LC          - Exponential number of loops to run the test.
  @param[in]     SOE         - Error handling switch for test.
  @param[in]     EnCADB      - Switch to enable CADB
  @param[in]     EnCKE       - Switch to enable CKE.
...
/**
  This function sets up a basic static WR/RD test for the given mask and per Outputs->McChBitMask.
  McChBitMask is initialized in MrcPretraining based on population, and used throughout internal CPGC structure.
  Modify McChBitMask to specify which MC/CH to program.
...
/**
  ...
  @param[in,out] MrcData             - Include all MRC global data.
  @param[in]     McChannelMask       - Channel mask to update Vref for.
...
MrcRelaxReadToReadSameRank (
  IN  MrcParameters *const MrcData,
  IN BOOLEAN               Relax,
  IN INT64          *const RdRdsg,
  IN INT64          *const RdRddg
  );
...
/**
  ...
  their CA ODT termination for the non-terminating ranks.  This is done across all LPDDR4 and LPDDR5 channels.
...
/**
  Switch LPDDR4 and LPDDR5 to high operating frequency:
  Set FSP_OP = 1
