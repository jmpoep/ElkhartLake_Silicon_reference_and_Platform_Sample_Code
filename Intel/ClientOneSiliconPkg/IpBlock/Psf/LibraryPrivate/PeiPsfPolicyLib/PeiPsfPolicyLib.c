/** @file
  This file provides services for Psf policy function
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, " TccEnable: 0x%X\n", PsfConfig->TccEnable));
}
...
STATIC COMPONENT_BLOCK_ENTRY  mPsfBlocks = {
  &gPsfConfigGuid,
  sizeof (PSF_CONFIG),
  PSF_CONFIG_REVISION,
  PsfLoadConfigDefault
...
**/
UINT16
PsfGetConfigBlockTotalSize (
  VOID
  )
{
  return mPsfBlocks.Size;
}
...
{
  return AddComponentConfigBlocks (ConfigBlockTableAddress, &mPsfBlocks, 1);
}
