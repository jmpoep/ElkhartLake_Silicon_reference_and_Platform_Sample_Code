...
#define B_PCH_SERIAL_IO_UART_LSR_BREAK    BIT4
...
{
  UINTN            BaseAddress;
  BaseAddress = FindSerialIoBar (UartNumber + PchSerialIoIndexUart0, 0);

  if (MmioRead32 (BaseAddress + R_PCH_SERIAL_IO_32BIT_UART_CTR) == UART_COMPONENT_IDENTIFICATION_CODE) {
  ...
  )
{
  ...
  // Read the serial port status
  //
  if ((ReadRegister (AccessMode, Base, R_PCH_SERIAL_IO_8BIT_UART_LSR) & B_PCH_SERIAL_IO_UART_LSR_BREAK) != 0) {
    return TRUE;
  }
  return FALSE;
}
