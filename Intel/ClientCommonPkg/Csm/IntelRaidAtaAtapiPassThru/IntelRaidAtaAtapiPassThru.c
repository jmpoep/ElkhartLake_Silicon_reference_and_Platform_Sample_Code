/** @file
  RAID mode of ATA host controller driver.
...
#include "IntelRaidAtaAtapiPassThru.h"

//
//  EFI_DRIVER_BINDING_PROTOCOL instance
//
EFI_DRIVER_BINDING_PROTOCOL gIntelRaidAtaAtapiPassThruDriverBinding = {
  IntelRaidAtaAtapiPassThruSupported,
  IntelRaidAtaAtapiPassThruStart,
  IntelRaidAtaAtapiPassThruStop,
  0x10,
  NULL,
  NULL
};

ATA_ATAPI_PASS_THRU_INSTANCE gIntelRaidAtaAtapiPassThruInstanceTemplate = {
  ...
  {                   // AtaPassThruMode
    //
    // According to UEFI2.3.1 spec Section 12.11, Drivers for RAID ATA controllers that
    // allow access the physical devices should set EFI_ATA_PASS_THRU_ATTRIBUTES_PHYSICAL
    // bit.
...
EFI_STATUS
EFIAPI
InitializeIntelRaidAtaAtapiPassThru (
  ...
  )
{
  ...
  Status = EfiLibInstallDriverBindingComponentName2 (
             ImageHandle,
             SystemTable,
             &gIntelRaidAtaAtapiPassThruDriverBinding,
             ImageHandle,
             &gIntelRaidAtaAtapiPassThruComponentName,
             &gIntelRaidAtaAtapiPassThruComponentName2
             );
  ...
  )
{
  ...
  // There is NO hard dependency of Legacy BIOS, we just want to make sure this driver
  // only work with IntelRaidBiosThunk on CSM ON mode.
  // For CSM OFF, we should have native RAID driver to handle that.
  //
  ...
  )
{
  ...
  //
  Instance = AllocateCopyPool (sizeof (ATA_ATAPI_PASS_THRU_INSTANCE), &gIntelRaidAtaAtapiPassThruInstanceTemplate);
  if (Instance == NULL) {
    goto ErrorExit;
  }
  ...
  )
{
  ...
  EFI_PCI_IO_PROTOCOL               *PciIo;
  EFI_RAID_REGISTERS                *AhciRegisters;
  UINT64                            Supports;
  ...
  AhciRegisters = &Instance->RaidRegisters;
  PciIo->Unmap (
           PciIo,
...
/**
  Enumerate all attached ATA devices at RAID mode.
  ...
  )
{
  ...
  Status = RaidModeInitialization (Instance);
  if (EFI_ERROR (Status)) {
  ...
  ) 
{
  ...
  AhciSaveContext (Instance->PciIo, &Instance->RaidRegisters, (UINT8)Port, (UINT8)PortMultiplierPort);
  switch (Protocol) {
    case EFI_ATA_PASS_THRU_PROTOCOL_ATA_NON_DATA:
      Status = AhciNonDataTransfer(
                 Instance->PciIo,
                 &Instance->RaidRegisters,
                 (UINT8)Port,
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "Execute Ata command 0x[%x] fails! Status=%r\n", Packet->Acb->AtaCommand, Status));
  }
  AhciRestoreContext (Instance->PciIo, &Instance->RaidRegisters, (UINT8)Port, (UINT8)PortMultiplierPort);
  return Status;
}
  ...
  )
{
  ...
  AhciSaveContext (Instance->PciIo, &Instance->RaidRegisters, (UINT8)Port, (UINT8)PortMultiplier);
  Status = AhciPacketCommandExecute (Instance->PciIo, &Instance->RaidRegisters, Port, PortMultiplier, Packet);
  AhciRestoreContext (Instance->PciIo, &Instance->RaidRegisters, (UINT8)Port, (UINT8)PortMultiplier);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "Execute Atapi command 0x[%x] fails Status=%r\n", ((UINT8 *)Packet->Cdb)[0], Status));
  }
