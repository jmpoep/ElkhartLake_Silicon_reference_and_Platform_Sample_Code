/** @file
  Produce FMP instances for PSE TSN Config Sub Region
...
#define TSN_CONFIG_FMP_PRIVATE_DATA_SIGNATURE      SIGNATURE_32('T', 'S', 'N', 'C')
#define TSN_CONFIG_FV_IMAGE_ID                     SIGNATURE_64('T', 'S', 'N','C','O','N','F','G')
#define TSN_CONFIG_FMP_LAST_ATTEMPT_VARIABLE_NAME  L"TsnConfigLastAttempVar"

FLASH_SUB_REGION_INFO mTsnConfigSpiSubRegion = {
  &gFmpCapsuleTsnConfigFileGuid,
  FixedPcdGet32 (PcdFlashFvTsnConfigOffset),
  ...
  )
{
  ...
  *Size = FixedPcdGet32 (PcdFlashFvTsnConfigSize);
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  //
  if (*ImageSize < PcdGet32 (PcdFlashFvTsnConfigSize)) {
    *ImageSize = PcdGet32 (PcdFlashFvTsnConfigSize);
  ...
  *ImageSize = PcdGet32 (PcdFlashFvTsnConfigSize);
  CopyMem (
    Image,
    (VOID *)(UINTN) ((PcdGet32 (PcdFlashFvTsnConfigOffset) + PcdGet32 (PcdBiosAreaBaseAddress)) & (UINT64)~B_FMP_HIGH_32BIT_MASK),
    *ImageSize
    );
  ...
  )
{
  ...
  Status = SubRegionWrite ( (VOID *) Image,
                          ImageSize,
                          TSN_CONFIG_FMP_PRIVATE_DATA_SIGNATURE,
                          &mTsnConfigSpiSubRegion,
                          AbortReason
                          );
  ...
  )
{
  ...
  *ImageUpdateable = IMAGE_UPDATABLE_VALID;
  ...
  /*
  if (!IsImageForSubRegion ((VOID *) Image, ImageSize, TSN_CONFIG_SUB_REGION_TYPE)) {
    *ImageUpdateable = IMAGE_UPDATABLE_INVALID;
  }
  */
...
{
  DEBUG ((DEBUG_INFO, "FmpDeviceLib(TsnConfig): FmpDeviceLock() for system FLASH\n"));
...
EFI_STATUS
EFIAPI
FmpDeviceLibTsnConfigConstructor (
