/** @file
  This file provides services to configure PCH Trace Hub PreMem policy
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, " AetEnabled  : 0x%x\n", PchTraceHubPreMemConfig->TraceHub.AetEnabled));
}

/**
  Load PCH Config block default
  ...
  )
{
  ...
  PchTraceHubPreMemConfig->TraceHub.AetEnabled = TRUE;
}

STATIC COMPONENT_BLOCK_ENTRY  mPchTraceHubPreMemBlocks = {
...
{
  return mPchTraceHubPreMemBlocks.Size;
}
...
{
  return AddComponentConfigBlocks (ConfigBlockTableAddress, &mPchTraceHubPreMemBlocks, 1);
}