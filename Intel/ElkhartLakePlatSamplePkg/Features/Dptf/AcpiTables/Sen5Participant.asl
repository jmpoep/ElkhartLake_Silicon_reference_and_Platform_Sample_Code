...
#endif //ECLITE_PSE_SUPPORT
{
  ...
  Device(SEN5)
  {
...
#endif
#ifdef ECLITE_PSE_SUPPORT
     \_SB.ECLT.ECLC(ECLITE_READ_COMMAND, ECLITE_DEFAULT_UPDATE, ECLITE_S3T1_OFFSET, ECLITE_BYTES_COUNT_2)
     Return(\_SB.IETM.C10K(\_SB.ECLT.ECLR( RefOf (\_SB.ECLT.S3T1))))
#endif //ECLITE_PSE_SUPPORT
    }
    ...
    Method(PAT0,1,Serialized)
    {
     ...
     \_SB.ECLT.ECLW(Local0, RefOf(\_SB.ECLT.S3A1))
     \_SB.ECLT.ECLC(ECLITE_WRITE_COMMAND, ECLITE_THERMAL_ALERT_THERHSOLD_UPDATE, ECLITE_S3A1_OFFSET, ECLITE_BYTES_COUNT_2)
#endif //ECLITE_PSE_SUPPORT
    }
...
#ifdef ECLITE_PSE_SUPPORT
     Store(\_SB.IETM.K10C(Arg0),Local0)
     \_SB.ECLT.ECLW(Local0, RefOf(\_SB.ECLT.S3A3))
     \_SB.ECLT.ECLC(ECLITE_WRITE_COMMAND, ECLITE_THERMAL_ALERT_THERHSOLD_UPDATE, ECLITE_S3A3_OFFSET, ECLITE_BYTES_COUNT_2)
#endif //ECLITE_PSE_SUPPORT
    }
    ...
    Method(_AC0,0,Serialized)
    {
      If(CTYP)
      {
        If (LEqual(\S5PT,0)) {Return(0xFFFFFFFF)} // Indicate DPTF driver to disable trip point
        Store(\_SB.IETM.CTOK(\S5PT),Local1) // Passive Cooling Policy
      } Else {
        If (LEqual(\S5AT,0)) {Return(0xFFFFFFFF)} // Indicate DPTF driver to disable trip point
    ...
    {
      If (LEqual(\S5CT,0)) {Return(0xFFFFFFFF)} // Indicate DPTF driver to disable trip point
    ...
    {
      If (LEqual(\S5S3,0)) {Return(0xFFFFFFFF)} // Indicate DPTF driver to disable trip point
    ...
    {
      If (LEqual(\S5HT,0)) {Return(0xFFFFFFFF)} // Indicate DPTF driver to disable trip point
