...
/**
  ...
  @param    PciIo                 The PCI IO protocol instance.
  @param    AhciRegisters         The pointer to the EFI_RAID_REGISTERS.
  @param    Port                  The number of port.
...
AhciBuildCommand (
  IN     EFI_PCI_IO_PROTOCOL        *PciIo,
  IN     EFI_RAID_REGISTERS         *AhciRegisters,
  IN     UINT8                      Port,
  ...
  )
{
  ...
  if ((PrdtNumber > 65535) || (PrdtNumber == 0)) {
    return;
  }
  ...
  )
{
  ...
  Data64.Uint32.Upper32 = AhciReadReg(PciIo, Offset);
  AhciCmdList = Data64.Uint64;
  Data64.Uint64 = (UINTN)AhciRegisters->AhciCmdListPciAddr;
  if (AhciCmdList != Data64.Uint64) {
    Offset = EFI_AHCI_PORT_START + Port * EFI_AHCI_PORT_REG_WIDTH + EFI_AHCI_PORT_CLB;
    AhciWriteReg (PciIo, Offset, Data64.Uint32.Lower32);
    Offset = EFI_AHCI_PORT_START + Port * EFI_AHCI_PORT_REG_WIDTH + EFI_AHCI_PORT_CLBU;
    AhciWriteReg (PciIo, Offset, Data64.Uint32.Upper32);
...
AhciCreateTransferDescriptor (
  IN     EFI_PCI_IO_PROTOCOL    *PciIo,
  IN OUT EFI_RAID_REGISTERS     *AhciRegisters
  )
{
  EFI_STATUS            Status;
  UINTN                 Bytes;
  ...
  //
  Capability           = AhciReadReg(PciIo, EFI_AHCI_CAPABILITY_OFFSET);
  MaxPortNumber        = PcdGet8 (PcdAhciMaxPorts);
  //
  ...
  )
{
  ...
  for (Port = 0; Port < MaxPortNumber; Port ++) {
    if ((PortImplementBitMap & (BIT0 << Port)) != 0) {
      ...
      DEBUG ((DEBUG_INFO, "port [%d] port mulitplier [%d] has a [%a]\n",
