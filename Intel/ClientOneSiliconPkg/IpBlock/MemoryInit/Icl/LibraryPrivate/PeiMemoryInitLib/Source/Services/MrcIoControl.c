  ...
  )
{
  ...
  MrcGetSetDdrIoGroupChannelStrobe (MrcData, MAX_CHANNEL, MAX_SDRAM_IN_DIMM, GsmIocInternalClocksOn, ReadForceWriteNoCache, &GetSetVal);

  MrcGetSetDdrIoGroupController0 (MrcData, GsmIocIoReset, ReadForceWriteNoCache, &GetSetVal);

  // Wait until the bit is cleared by hardware
  do {
    MrcGetSetDdrIoGroupController0 (MrcData, GsmIocIoReset, ReadNoCache, &GetSetVal);
    Busy  = (GetSetVal == 1);
