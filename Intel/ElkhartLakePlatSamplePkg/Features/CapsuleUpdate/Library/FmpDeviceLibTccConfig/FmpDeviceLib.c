/** @file
  Produce FMP instances for TCC Config Sub Region
...
#define TCC_CONFIG_FMP_PRIVATE_DATA_SIGNATURE      SIGNATURE_32('T', 'C', 'C', 'C')
#define TCC_CONFIG_FV_IMAGE_ID                     SIGNATURE_64('T', 'C', 'C','C','O','N','F','G')
#define TCC_CONFIG_FMP_LAST_ATTEMPT_VARIABLE_NAME  L"TccConfigLastAttempVar"

FLASH_SUB_REGION_INFO mTccConfigSpiSubRegion = {
  &gFmpCapsuleTccConfigFileGuid,
  FixedPcdGet32 (PcdFlashFvTccConfigOffset),
  ...
  )
{
  ...
  *Size = FixedPcdGet32 (PcdFlashFvTccConfigSize);
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  //
  if (*ImageSize < PcdGet32 (PcdFlashFvTccConfigSize)) {
    *ImageSize = PcdGet32 (PcdFlashFvTccConfigSize);
  ...
  *ImageSize = PcdGet32 (PcdFlashFvTccConfigSize);
  CopyMem (
    Image,
    (VOID *)(UINTN) ((PcdGet32 (PcdFlashFvTccConfigOffset) + PcdGet32 (PcdBiosAreaBaseAddress)) & (UINT64)~B_FMP_HIGH_32BIT_MASK),
    *ImageSize
    );
  ...
  )
{
  ...
  Status = SubRegionWrite ( (VOID *) Image,
                          ImageSize,
                          TCC_CONFIG_FMP_PRIVATE_DATA_SIGNATURE,
                          &mTccConfigSpiSubRegion,
                          AbortReason
                          );
  ...
  )
{
  ...
  *ImageUpdateable = IMAGE_UPDATABLE_VALID;
  ...
  /*
  if (!IsImageForSubRegion ((VOID *) Image, ImageSize, TCC_CONFIG_SUB_REGION_TYPE)) {
    *ImageUpdateable = IMAGE_UPDATABLE_INVALID;
  }
  */
...
{
  DEBUG ((DEBUG_INFO, "FmpDeviceLib(TccConfig): FmpDeviceLock() for system FLASH\n"));
...
EFI_STATUS
EFIAPI
FmpDeviceLibTccConfigConstructor (
