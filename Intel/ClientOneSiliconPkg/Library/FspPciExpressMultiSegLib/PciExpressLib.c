...
/**
  ...
  only in the low 29 bits.
  BIT28 indicate the segment number, currently only support Segment0 and Segment1

  @param  A The address to validate.

**/
#define ASSERT_INVALID_PCI_ADDRESS(A) \
  ASSERT (((A) & ~0x1fffffff) == 0)
...
/**
  ...
  PCD index is different between Fsp and  boot loader. We need a put this PCD
  in PcdsDynamicEx section to make sure base address is correct.
  FSP notify phase code and library should use this library.
...
{
  return (VOID*)(UINTN) PcdGet64 (PcdSiPciExpressBaseAddress);
}
