  ...
  )
{
  ...
  for (; BufferSize > 0;) {
    ...
    //
    // INT13 does not allow the segment of transfer buffer is increased 
    // (e.g. offset + transferring bytes should not exceed 65536), 
    // otherwise INT13 will return 09 data boundary error per INT13 spec.  
    // TransferBuffer has been 4KB aligned, apply the algorithm below to 
    // keep offset is 0 to make INT13 transfer max 127 block successfully.
    //
