...
/**
  Empty Function to trigger OS Dnx Call back event
...
/**
Upon failure to boot OS bootloader, trigger OS Dnx.
...
)
{
  ...
  //
  // Update Variable for the Dnx driver to understand Trigger is for Boot Loader Corruption.
  //
  ...
  //
  // Upon failure to boot to bootloader, BIOS shall boot Dnx
  //
  ...
  if (!EFI_ERROR(Status)) {
    gBS->SignalEvent(DnxEvent);
    gBS->CloseEvent(DnxEvent);
  }
}
  ...
  )
{
  ...
  EFI_DEVICE_PATH_PROTOCOL  *RamDiskDevicePath;
  EFI_DEVICE_PATH_PROTOCOL  *UfsDevicePath;
  VOID                      *FileBuffer;
  UINTN                     FileSize;
  ...
  if (DevicePathType (BootOption->FilePath) != BBS_DEVICE_PATH) {
    ...
    if (EFI_ERROR (Status)) {
      ...
      //
      // Check for Ufs Device Path to initiate recovery
      //
      UfsDevicePath = BootOption->FilePath;
      while (!IsDevicePathEndType (NextDevicePathNode (UfsDevicePath))) {
        UfsDevicePath = NextDevicePathNode (UfsDevicePath);
        if (UfsDevicePath->Type == MESSAGING_DEVICE_PATH && UfsDevicePath->SubType == MSG_UFS_DP) {
          DEBUG ((EFI_D_INFO, "[Bds] Found UFS Boot Option,  Trigger Os Recovery"));
          BootFailTriggerDnx ();
          break;
        }
      }
