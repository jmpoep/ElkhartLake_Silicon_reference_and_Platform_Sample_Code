...
typedef struct {
  ...
  DWC_XDCI_TRB             unaligned_trbs [(DWC_XDCI_MAX_ENDPOINTS * 2 + 1) * DWC_XDCI_TRB_NUM]; // TRBs. EndPoint number include in/out that would be 16 * 2
  DWC_XDCI_TRB             *trbs;                                               // 16-bytes aligned TRBs.
  DWC_XDCI_ENDPOINT        ep_handles [DWC_XDCI_MAX_ENDPOINTS * 2];             // EPs number include in/out that would be 16 * 2
