...
XCOPY /I /S /Y %WORKSPACE_SILICON%\ClientOneSiliconPkg\Override\ClientSiliconPkg\Library\PeiPerformanceLibFpdt %WORKSPACE_COMMON%\ClientSiliconPkg\Library\PeiPerformanceLibFpdt

pushd %WORKSPACE_SILICON%\ClientOneSiliconPkg
python gen_dec.py SiPkg.dec.template JasperLake
popd
...
@if not defined IASL_PREFIX_EXT (
  @set IASL_PREFIX_EXT=%WORKSPACE_PLATFORM%\JasperLakePlatSamplePkg\Tools\Iasl\
)
...
@echo ACTIVE_PLATFORM = %WORKSPACE_SILICON_RELATIVE%/ClientOneSiliconPkg/Product/JasperLake/JasperLakeSiliconPkg.dsc  >> Build\ClientOneSiliconPkg\%TARGET%_%TOOL_CHAIN_TAG%\target.txt
