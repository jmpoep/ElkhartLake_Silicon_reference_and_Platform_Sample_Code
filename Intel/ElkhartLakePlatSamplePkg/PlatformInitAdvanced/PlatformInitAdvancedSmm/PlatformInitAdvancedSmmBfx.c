...
/**
  ...
  @param[in]       Context       A pointer given when SmmSetVariable was called. The callback may use this to
...
VOID
EFIAPI
SetupCpuFeaturesSetVariableCallback (
  IN      VOID            *Context,    OPTIONAL
  IN      EFI_STATUS      Status
  )
{
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "Failed to set the SetupCpuFeatures variable.\n"));
  }
}
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "Failed to set the RegParm variable.\n"));
  }

  InitialData = V_PCH_IO_RST_CNT_HARDSTARTSTATE;
  OutputData = V_PCH_IO_RST_CNT_HARDRESET;
  IoWrite8 ((UINTN) R_PCH_IO_RST_CNT, (UINT8) InitialData);
  IoWrite8 ((UINTN) R_PCH_IO_RST_CNT, (UINT8) OutputData);
  CpuDeadLoop ();
  ASSERT (FALSE);
}
...
VOID
EFIAPI
OfflineCpuContextSetVariableCallback (
  IN      VOID            *Context,    OPTIONAL
  IN      EFI_STATUS      Status
  )
{
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "Failed to set the OfflineCpuContext variable.\n"));
  }
...
//  ASSERT_EFI_ERROR (Status);
  ...
  //
  // Locate EdkiiSmmVariableProtocol
  //
  ...
  //
  // TEMP_BFX_ENABLING: Do not set SetupCpuFeatures on UFS boot since it could potentially lock the BIOS
  //                    Storage Proxy. Move outside SMM if possible and call SetVariable().
  //
  if (!BootMediaIsUfs ()) {
    ASSERT (mEdkiiSmmVariable != NULL);
    if (mEdkiiSmmVariable != NULL) {
      Status = mEdkiiSmmVariable->SmmSetVariable (
                                    NULL,
                                    L"SetupCpuFeatures",
                                    &gSetupVariableGuid,
                                    VariableAttr,
      ...
      if (EFI_ERROR (Status)) {
        DEBUG ((DEBUG_ERROR, "Failed to register the SetupCpuFeatures variable to be set.\n"));
      }
    }
  }
