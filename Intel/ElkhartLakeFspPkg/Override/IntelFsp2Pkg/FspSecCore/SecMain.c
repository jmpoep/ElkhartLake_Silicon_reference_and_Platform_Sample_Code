...
#include <Library/HobLib.h> //FSPO-0013: RoyalParkOverride
  ...
  )
{
  ...
  //
  // memory map when running on bootloader stack
  // |-------------------|---->
  // |Idt Table          |
  // |-------------------|
  // |PeiService Pointer |
  ...
  // memory map when running FSP on a separate stack
  //
  ...
  if (PcdGet8 (PcdFspHeapSizePercentage) == 0) {
    SecCoreData.TemporaryRamSize       = SizeOfRam; // size to copy to the permanent memory
    SecCoreData.PeiTemporaryRamBase    = SecCoreData.TemporaryRamBase;
    SecCoreData.PeiTemporaryRamSize    = SecCoreData.TemporaryRamSize; // FSPO-0013: Override
    SecCoreData.StackBase              = (VOID *)GetFspEntryStack(); // FSPO-0013: Override
