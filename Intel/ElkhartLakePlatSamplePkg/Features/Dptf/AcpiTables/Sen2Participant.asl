...
#endif //ECLITE_PSE_SUPPORT
{
  ...
  Device(SEN2)
  {
    ...
    Name(_UID, "SEN2")
    Name(_STR, Unicode ("Thermistor SOC")) // Placed close to SOC
    Name(PTYP, 0x03)
    Name(CTYP,0)  // Mode
    // PFLG
...
#endif
#ifdef ECLITE_PSE_SUPPORT
     \_SB.ECLT.ECLC(ECLITE_READ_COMMAND, ECLITE_DEFAULT_UPDATE, ECLITE_S0T1_OFFSET, ECLITE_BYTES_COUNT_2)
     Return(\_SB.IETM.C10K(\_SB.ECLT.ECLR( RefOf (\_SB.ECLT.S0T1))))
#endif //ECLITE_PSE_SUPPORT
    }
    ...
    Method(PAT0, 1, Serialized)
    {
     ...
     \_SB.ECLT.ECLW(Local0, RefOf(\_SB.ECLT.S0A1 ))
     \_SB.ECLT.ECLC(ECLITE_WRITE_COMMAND, ECLITE_THERMAL_ALERT_THERHSOLD_UPDATE, ECLITE_S0A1_OFFSET, ECLITE_BYTES_COUNT_2)
#endif //ECLITE_PSE_SUPPORT
    }
...
#ifdef ECLITE_PSE_SUPPORT
     Store(\_SB.IETM.K10C(Arg0),Local0)
     \_SB.ECLT.ECLW(Local0, RefOf(\_SB.ECLT.S0A3 ))
     \_SB.ECLT.ECLC(ECLITE_WRITE_COMMAND, ECLITE_THERMAL_ALERT_THERHSOLD_UPDATE, ECLITE_S0A3_OFFSET, ECLITE_BYTES_COUNT_2)
#endif //ECLITE_PSE_SUPPORT
    }
...
#endif
#ifdef ECLITE_PSE_SUPPORT
     Notify(\_SB.SEN2, 0x91) // notify the participant of a trip point change event
#endif //ECLITE_PSE_SUPPORT
    }
    ...
    {
      If (LEqual(\S2CT,0)) {Return(0xFFFFFFFF)} // Indicate DPTF driver to disable trip point
    ...
    {
      If (LEqual(\S2S3,0)) {Return(0xFFFFFFFF)} // Indicate DPTF driver to disable trip point
    ...
    {
      If (LEqual(\S2HT,0)) {Return(0xFFFFFFFF)} // Indicate DPTF driver to disable trip point
