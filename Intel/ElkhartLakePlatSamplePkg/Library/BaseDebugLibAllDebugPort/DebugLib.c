/** @file
  Base Debug library instance base on all Debug Port library.
  It uses PrintLib to send debug messages to all Debug Port.
...
/**
  The constructor function initialize all Debug Port Library
...
RETURN_STATUS
EFIAPI
BaseDebugLibAllDebugPortConstructor (
  VOID
  )
{
  ...
  )
{
  ...
  if ((StatusCodeFlags & STATUS_CODE_USE_TRACEHUB) &&
      (PcdGetBool (PcdStatusCodeUseTraceHub) == TRUE)) {
    //
  ...
  if (StatusCodeFlags & STATUS_CODE_USE_RAM) {
    RamDebugWrite ((UINT8 *)Buffer, AsciiStrLen (Buffer));
  }
