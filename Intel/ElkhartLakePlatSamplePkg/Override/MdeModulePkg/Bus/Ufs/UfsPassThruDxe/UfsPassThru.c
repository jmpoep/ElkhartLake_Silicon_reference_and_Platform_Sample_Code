...
{
  EFI_STATUS  Status;
  UINT32  DeviceInitStatus;
  UINT16  Timeout;

  DeviceInitStatus = 0xFFFFFFFF;
  ...
  MicroSecondDelay (100); //Give 100 us and then start polling, there are cards that can take upto 600ms as per validation/CSE team.
  Timeout = 6000;
  do {
  ...
  //
  if (Timeout == 0) {
    DEBUG ((EFI_D_ERROR, "UfsFinishDeviceInitialization DeviceInitStatus=%x EFI_TIMEOUT \n", DeviceInitStatus));
    return EFI_TIMEOUT;
  } else {
    DEBUG ((EFI_D_INFO, "UfsFinishDeviceInitialization Timeout left=%x EFI_SUCCESS \n", Timeout));
    return EFI_SUCCESS;
  }
  //
  ...
  )
{
  ...
  UFS_DEV_DESC                          DeviceDescriptor;     // RPPO-LKF-0018: Override
  UINT32                                UnitDescriptorSize;   // RPPO-LKF-0023: Override
  UINT32                                Attributes;           // RPPO-LKF-0036: Override
  UINT8                                 RefClkAttr;           // RPPO-LKF-0036: Override
  ...
  InitializeListHead (&Private->PendingQueue);
  ...
  for (Index = 10; Index > 0; Index++) {
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((EFI_D_ERROR, "Ufs Sending NOP IN command Error, Index = %x Status = %r\n", Index, Status));
      MicroSecondDelay (100); //100 us
      continue;
    } else {
      DEBUG ((EFI_D_INFO, "Ufs Sent NOP OUT successfully and received NOP IN, Status = %r\n", Status));
      break;
    }
  }
  if (!Index) {
    DEBUG ((EFI_D_INFO, "NOP OUT failed all the %d times Status = %r\n", Index, Status));
    goto Error;
  }

  Status = UfsFinishDeviceInitialization (Private);
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "Device failed to finish initialization, Status = %r\n", Status));
    goto Error;
  }
  ...
  //
  UnitDescriptorSize = sizeof (UFS_UNIT_DESC);            // RPPO-LKF-0023: Override
  for (Index = 0; Index < 8; Index++) {
    Status = UfsRwDeviceDesc (Private, TRUE, UfsUnitDesc, (UINT8) Index, 0, &UnitDescriptor, &UnitDescriptorSize);
    if (EFI_ERROR (Status)) {
      DEBUG ((EFI_D_ERROR, "Failed to read unit descriptor, index = %X, status = %r\n", Index, Status));
      continue;
    }
    //
    // RPPO-LKF-0034: OverrideBegin
    //
    if (UnitDescriptor.LunEn != 0x0) {
      DEBUG ((EFI_D_INFO, "UFS LUN %X is enabled\n", Index));
      Private->Luns.BitMask |= (BIT0 << Index);
      if (UnitDescriptor.LunEn == 0x2) {
        DEBUG ((EFI_D_INFO, "  Host Performance Booster enabled\n"));
      }
    }
    //
    // RPPO-LKF-0034: OverrideEnd
    //
  }

  //
  // RPPO-LKF-0018: OverrideBegin
  //
  //
  // Check if RPMB is supported and set corresponding bit mask.
  //
  Status = UfsRwDeviceDesc (Private, TRUE, UfsDeviceDesc, 0, 0, &DeviceDescriptor, &UnitDescriptorSize);
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "Failed to read device descriptor, status = %r\n", Status));
  } else {
    if (DeviceDescriptor.SecurityLun == 0x1) {
      DEBUG ((EFI_D_INFO, "UFS WLUN RPMB is supported\n"));
      Private->Luns.BitMask |= BIT11;
    }
  }
  //
  // RPPO-LKF-0018: OverrideEnd
  //
  //
  // RPPO-LKF-0036: OverrideBegin
  //
  ...
  Attributes = 0;
  Status = UfsRwAttributes (Private, TRUE, RefClkAttr, 0, 0, &Attributes);
  if (!EFI_ERROR (Status)) {
    DEBUG ((EFI_D_INFO, "AttributeID=%x Attributes=%x 0 -> 19.2Mhz  1 -> 26Mhz 2 -> 38.4 Mhz 3 -> Obsolete Others -> Reserved \n", RefClkAttr, Attributes));
    if (Attributes != 0) {
      Attributes = 0;
      Status = UfsRwAttributes (Private, FALSE, RefClkAttr, 0, 0, &Attributes);
      if (EFI_ERROR (Status)) {
        DEBUG ((EFI_D_ERROR, "Failed to Change Reference Clock Attribute to 19.2 Mhz, Status = %r \n", Status));
      }
    }
  } else {
    DEBUG ((EFI_D_ERROR, "Failed to Read Reference Clock Attribute, Status = %r \n", Status));
  }
  //
  // RPPO-LKF-0036: OverrideEnd
  //
  ...
  if (!BootMediaIsUfs ()) {
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "Failed to switch the Power Mode and Gear\n"));
      return Status;
    }
  }
  ...
  )
{
  ...
  if (!IsListEmpty(&Private->PendingQueue)) {
    EFI_LIST_FOR_EACH_SAFE (Entry, NextEntry, &Private->PendingQueue) {
      TransReq  = UFS_PASS_THRU_TRANS_REQ_FROM_THIS (Entry);
      ...
      //
      // No need to free up other resources, the request not even started.
      //
      gBS->SignalEvent (TransReq->CallerEvent);
      FreePool (TransReq);
    }
  }
