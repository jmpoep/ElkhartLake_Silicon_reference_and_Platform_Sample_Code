/** @file
  This file contains ICL specific FIA routines
  ...
  )
{
  ...
  if (IsPchN ()) {
    ...
    if (PchFiaGetLaneOwner (FiaInst, 12) == PchFiaOwnerGbe) return 13;
    if (PchFiaGetLaneOwner (FiaInst, 13) == PchFiaOwnerGbe) return 14;
  } else {
    ASSERT (IsPchH ());
  ...
  )
{
  ...
  //On ICP-N Fia-PCIE Port 0-3 are connected to SPB Port 0-3 and Fia-PCIE Port 4-7 are connected to SPA Port 0-3, so we modify ClkReqMap to handle this.
  if (IsPchN ()) {
    for (Index = 0; Index < (ClkReqMapSize / 2); Index++) {
      Temp = ClkReqMap[Index];
      ClkReqMap[Index] = ClkReqMap[Index + (ClkReqMapSize / 2)];
      ClkReqMap[Index + (ClkReqMapSize / 2)] = Temp;
    }
  }
  ...
  )
{
  ...
  PchPcrAndThenOr32 (
    ...
    // Should not set PCR[FIA] + 0h bit [31] for OSIdle support on PCH-N and PCH-H
    //
    PchPcrAndThenOr32 (
      FiaInst.SbPortId,
      R_PCH_FIA_PCR_CC,
