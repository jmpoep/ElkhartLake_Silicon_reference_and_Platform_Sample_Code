/** @file
  DXE Multi-Board Initialization Library
...
BOARD_NOTIFICATION_INIT_FUNC mEhlBoardDxeInitFunc = {
  EhlBoardInitAfterPciEnumeration,
  EhlBoardInitReadyToBoot,
  EhlBoardInitEndOfFirmware
};
...
EFI_STATUS
EFIAPI
DxeEhlMultiBoardInitLibConstructor (
  ...
  )
{
  if ((LibPcdGetSku () == BoardIdEhlLp4xType4Erb) ||
    ...
    return RegisterBoardNotificationInit (&mEhlBoardDxeInitFunc);
  }
  return EFI_SUCCESS;
}
