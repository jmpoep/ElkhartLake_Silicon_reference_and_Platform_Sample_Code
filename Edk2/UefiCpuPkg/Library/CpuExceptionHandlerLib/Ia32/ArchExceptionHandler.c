  ...
  )
{
  ...
  if ((mErrorCodeFlag & (1 << ExceptionType)) != 0) {
    ...
    if (ExceptionType == EXCEPT_IA32_PAGE_FAULT) {
      InternalPrintMessage (
        "  I:%x R:%x U:%x W:%x P:%x PK:%x SS:%x SGX:%x", // CCEO-0016: RoyalParkOverrideContent
        ...
        (SystemContext.SystemContextIa32->ExceptionData & IA32_PF_EC_SS)   != 0, // CCEO-0016: RoyalParkOverrideContent
