  ...
  )
{
  ...
  UINTN                              Lun; // SIPO-LKF-0001:OverrideContent
  ...
  Lun = DeviceIndex - 1; // SIPO-LKF-0001:OverrideContent
  ...
  )
{
  ...
  Status    = EFI_SUCCESS;
  NeedRetry = TRUE;
  Private   = GET_UFS_PEIM_HC_PRIVATE_DATA_FROM_THIS3 (This);
  ...
  DEBUG ((DEBUG_INFO, "Final status of UFS write == %r\n", Status));
  return Status;
}
  ...
  )
{
  ...
  while (TRUE) {
    ...
    Private->UfsWritePpiList.Ppi = &Private->UfsWritePpi;
    ...
    Status = PeiServicesInstallPpi (&Private->UfsWritePpiList);
    Controller++;
  }
