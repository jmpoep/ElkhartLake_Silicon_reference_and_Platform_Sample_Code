  ...
  )
{
  ...
  //
  DEBUG_CODE_BEGIN ();
  if (((GpioConfig->ElectricalConfig & B_GPIO_ELECTRICAL_CONFIG_1V8_TOLERANCE_MASK) >> N_GPIO_ELECTRICAL_CONFIG_1V8_TOLERANCE_BIT_POS) != GpioHardwareDefault) {
    ASSERT (FALSE); // PadTolerance is not supported on CNL-PCH
  }
  DEBUG_CODE_END ();
  ...
  )
{
  ...
  for (GpeDwXIndex = 0; GpeDwXIndex < 3; GpeDwXIndex++) {
    for (GroupIndex = 0; GroupIndex < GpioGroupInfoLength; GroupIndex++) {
      if (GpioGroupInfo[GroupIndex].PmcGpeDwx == PmcGpeDwXValue[GpeDwXIndex]) {
        GroupToGpeDwX[GpeDwXIndex] = GpioGetGroupFromGroupIndex (GroupIndex);
        break;
      }
    }
  }
  ...
  )
{
  ...
  //
  GpeDwXGroupIndex[0] = GpioGetGroupIndexFromGroup (GroupToGpeDw0);
  GpeDwXGroupIndex[1] = GpioGetGroupIndexFromGroup (GroupToGpeDw1);
  GpeDwXGroupIndex[2] = GpioGetGroupIndexFromGroup (GroupToGpeDw2);

  //
  // Check if group can be used as Tier1 GPE
  //
  for (GpeDwXIndex = 0; GpeDwXIndex < 3; GpeDwXIndex++) {
    if ((GpioGroupInfo[GpeDwXGroupIndex[GpeDwXIndex]].PmcGpeDwx == NO_REGISTER_FOR_PROPERTY) ||
       (GpioGroupInfo[GpeDwXGroupIndex[GpeDwXIndex]].GpioGpeDwx == NO_REGISTER_FOR_PROPERTY)) {
      DEBUG ((DEBUG_ERROR, "GPIO ERROR: Group %a cannot be enabled for 1-Tier GPE\n", GpioGetGroupName (GpeDwXGroupIndex[GpeDwXIndex])));
      return EFI_INVALID_PARAMETER;
    }
  }
  ...
  //
  PmcSetGpioGpe (
    GpioGroupInfo[GpeDwXGroupIndex[0]].PmcGpeDwx,
    GpioGroupInfo[GpeDwXGroupIndex[1]].PmcGpeDwx,
    GpioGroupInfo[GpeDwXGroupIndex[2]].PmcGpeDwx
    );
  ...
  Data32Or = (UINT32) ((GpioGroupInfo[GpeDwXGroupIndex[2]].GpioGpeDwx << N_GPIO_PCR_MISCCFG_GPE0_DW2) |
                       (GpioGroupInfo[GpeDwXGroupIndex[1]].GpioGpeDwx << N_GPIO_PCR_MISCCFG_GPE0_DW1) |
                       (GpioGroupInfo[GpeDwXGroupIndex[0]].GpioGpeDwx << N_GPIO_PCR_MISCCFG_GPE0_DW0));
