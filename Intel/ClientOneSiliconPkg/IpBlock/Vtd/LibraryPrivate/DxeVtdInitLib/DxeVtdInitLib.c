...
**/
#if FixedPcdGetBool(PcdITbtEnable) == 0
UINT16  mDevEnMap[][2] = {{0x0200, 0x80}, {0x1400, 0x80}, {0x1401, 0x80}};
#else
UINT16  mDevEnMap[][2] = {{0x0200, 0x80}, {0x0500, 0x80}, {0x1400, 0x80}, {0x1401, 0x80}, {0x0700, 0x80}, {0x0701, 0x80}, {0x0702, 0x80}, {0x0703, 0x80}};
#endif
  ...
  )
{
  ...
  ASSERT_EFI_ERROR (Status);
  DEBUG_CODE_BEGIN();
  DEBUG ((DEBUG_INFO, "mInterruptRemappingSupport: %x\n", mInterruptRemappingSupport));
  DEBUG ((DEBUG_INFO, "BaseAddress[0]: %x, Version 0x%x\n", mVtdDataHob->BaseAddress[0],MmioRead32(mVtdDataHob->BaseAddress[0])));
  if(mVtdDataHob->BaseAddress[0] != 0) {
    DEBUG ((DEBUG_INFO, "  Version 0x%x\n", MmioRead32(mVtdDataHob->BaseAddress[0])));
  }
  DEBUG ((DEBUG_INFO, "BaseAddress[1]: %x\n", mVtdDataHob->BaseAddress[1]));
  DEBUG ((DEBUG_INFO, "BaseAddress[2]: %x\n", mVtdDataHob->BaseAddress[2]));
  if(mVtdDataHob->BaseAddress[2] != 0) {
    DEBUG ((DEBUG_INFO, "  Version 0x%x\n", MmioRead32(mVtdDataHob->BaseAddress[2])));
  }
  DEBUG_CODE_END();
  ...
  DEBUG ((DEBUG_INFO, "DMAR flag: 0x%x\n", DmarTable->DmarHeader.Flags));
