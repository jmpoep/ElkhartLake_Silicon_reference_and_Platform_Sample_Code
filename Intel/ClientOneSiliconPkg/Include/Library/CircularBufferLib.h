/** @file
  Header file for the Circular Buffer Library.
...
#ifndef _CIRCULAR_BUFFER_LIB_H_
#define _CIRCULAR_BUFFER_LIB_H_

#include <Base.h>
#include <Uefi.h>
#include <Library/BaseLib.h>

#define CIRCULAR_BUFFER_DATA_ALIGN(Index,Alignment)  (((UINTN) (Index) + Alignment - 1) & (~(Alignment - 1)))

typedef struct {
  UINTN         Head;      ///< Current buffer head (write) position
  UINTN         Tail;      ///< Current buffer tail (read) position
  CONST  UINTN  Capacity;  ///< Maximum buffer size in bytes
  UINTN         Count;     ///< Number of items in the circular buffer
  UINT8         *Buffer;   ///< Pointer to the circular buffer
} CIRCULAR_BUFFER;
