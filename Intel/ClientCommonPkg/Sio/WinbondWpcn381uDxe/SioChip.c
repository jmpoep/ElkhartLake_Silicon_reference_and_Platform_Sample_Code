  ...
  )
{
  ...
  CopyMem (&LocalDevices[LocalCount], &mDeviceInfo[4].Device, sizeof (EFI_SIO_ACPI_DEVICE_ID));
  LocalCount ++;
...
{
  //
  // Identify the SIO by the Device ID.
  // Return EFI_UNSUPPORTED if the SIO device is not found.
  //
  IoWrite8 (SIO_INDEX_PORT, R_DEVICE_ID);   // Winbond WPCN381U Device ID register is 0x20
  if ((IoRead8 (SIO_DATA_PORT) != 0xF4) && (IoRead8 (SIO_DATA_PORT) != 0xF1))  {  // Winbond WPCN381U Device ID is 0xF4 , Winbond WPCD374LAUFG Device ID is 0xF1
