...
const TOdtValueLpddr Lpddr4OdtTable[MAX_DIMMS_IN_CHANNEL][2] = {
/// 1DPC 1R,  1DPC 2R
  {{40, 40},  {60, 40}},
};
  ...
  )
{
  ...
  // Static Values.
  Mr22->Bits.x8OdtDis7to0  = (MrcData->Outputs.LpByteMode) ? 1 : 0;
  ...
  )
{
  ...
  Debug         = &Outputs->Debug;
  Status = mrcSuccess;
  Profile = MrcData->Inputs.MemoryProfile;
  ...
  if ((Outputs->RxMode == MrcRxModeUnmatchedRxWPpath) || (Outputs->RxMode == MrcRxModeUnmatchedRxWRload)) {
    RdPreambleMode = Lp4RdPreToggle;
  } else {
    RdPreambleMode = Lp4RdPreStatic;
  }
  // Extract ODT tables from the first populated channel
  for (Controller = 0; (Controller < MAX_CONTROLLER) && (Status == mrcSuccess) && (OdtFound == FALSE); Controller++) {
    ControllerOut = &Outputs->Controller[Controller];
    for (Channel = 0; (Channel < MAX_CHANNEL) && (Status == mrcSuccess) && (OdtFound == FALSE); Channel++) {
      if (MrcChannelExist (MrcData, Controller, Channel)) {
        Lpddr4OdtTableIndex = (TOdtValueLpddr *)GetOdtTableIndex (MrcData, Controller, Channel, dDIMM0);
  ...
  for (Controller = 0; Controller < MAX_CONTROLLER; Controller++) {
    ...
    for (Channel = 0; (Channel < MAX_CHANNEL) && (Status == mrcSuccess); Channel++) {
      ...
      for (Rank = 0; (Rank < MAX_RANK_IN_CHANNEL) && (Status == mrcSuccess); Rank++) {
        ...
        Status = MrcLpddr4SetMr1 (MrcData, Lp4Bl32, Lp4WrPre2Tck, RdPreambleMode, (UINT8) TimingPtr->tWR, Lp4RdPst0_5Tck, &MrPtr[mrIndexMR1]); // Sambar, switch LP4 preamble to Lp4RdPreToggle from Lp4RdPreStatic
        if (Status == mrcSuccess) {
          RlVal = TimingPtr->tCL;
          // Subtract 1 here to take into account tDQSS.
          WrVal = TimingPtr->tCWL - 1;
          Status = MrcLpddr4SetMr2 (MrcData, RlVal, WrVal, Lp4WlSetB, Lp4WlModeDis, &MrPtr[mrIndexMR2]);
          if (Status == mrcSuccess) {
  ...
  )
{
  ...
  if (DebugPrint) {
    MRC_DEBUG_MSG (
      ...
      "MrcIssueMpc on Mc %d Ch %d Rank %d: Address 0x%X, Data=0x%08X\n",
  ...
  )
{
  ...
  for (Controller = 0; Controller < MAX_CONTROLLER; Controller++) {
    for (Channel = 0; Channel < MAX_CHANNEL; Channel++) {
      ...
      for (Rank = 0; Rank < MAX_RANK_IN_CHANNEL; Rank ++) {
        ...
        Data = ChannelOut->Dimm[dDIMM0].Rank[Rank % MAX_RANK_IN_DIMM].MR[mrIndexMR2];
  ...
  Status = MrcSetFspVrcg (MrcData, ALL_RANK_MASK, MRC_IGNORE_ARG_8, Lp4FspWePoint1, Lp4FspOpPoint0);
  if (Status != mrcSuccess) {
    return Status;
  }
      ...
      for (Rank = 0; Rank < MAX_RANK_IN_CHANNEL; Rank++) {
        ...
        for (Index = 0; Index < MRC_LPDDR4_INIT_NUM_MR; Index++) {
          if (MrIndex[Index] < MAX_MR_IN_DIMM) {
            MrData[Index] = (UINT8) ChannelOut->Dimm[dDIMM0].Rank[Rank % MAX_RANK_IN_DIMM].MR[MrIndex[Index]];
          } else {
...
/**
  Switch LPDDR4 and LP5 to high operating frequency:
   - Set FSP_OP = 1
  ...
  )
{
  ...
  // Set FSP-OP = 1
  Status = MrcSetFspVrcg (MrcData, ALL_RANK_MASK, MRC_IGNORE_ARG_8, MRC_IGNORE_ARG_8, LpFspOpPoint1);
  if (Status != mrcSuccess) {
    return Status;
  }
  ...
  )
{
  ...
  if (RankMask &  (1 << Rank)) {
    ...
    if (UpdateMrcData) {
      if (VrefType == CmdV) {
        Status = MrcLpddr4SetMr12 (MrcData, VrefCode, Range, 0, &MrPtr[mrIndexMR12]);
      } else {
  ...
  )
{
  ...
  OdtTable = (TOdtValueLpddr *) &Lpddr4OdtTable[Dimm][OdtIndex];
  ...
  OUT UINT8               *NumDimmOptParamVals)
{
  ...
  static const UINT16    Lpdr4DimmRttWrVals[] = { 0xFFFF, 240, 120, 80, 60, 48, 40 };      // LPDDR4 RttWr Values in Ohms
  MrcDebug               *Debug;
  MrcStatus              Status;
...
//      MRC_DEBUG_MSG (Debug, MSG_LEVEL_ERROR, "Lpddr4SetDramVref - IsCachedOffsetParam: Rank: %d, MR14: 0x%02X, CurentOffset: %d, Offset: %d\n", Rank, Lpddr4VrefMr.Data8, CurrentOffset, Offset);
      ...
      MrcSetVrefLpddr4 (MrcData, VrefType, Controller, Channel, Rank, CurrentOffset, UpdateMrcData);
    }
