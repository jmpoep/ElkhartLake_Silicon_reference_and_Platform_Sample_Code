...
#endif //ECLITE_PSE_SUPPORT
{
  ...
  Device(SEN3)
  {
...
#endif
#ifdef ECLITE_PSE_SUPPORT
     \_SB.ECLT.ECLC(ECLITE_READ_COMMAND, ECLITE_DEFAULT_UPDATE, ECLITE_S1T1_OFFSET, ECLITE_BYTES_COUNT_2)
     Return(\_SB.IETM.C10K(\_SB.ECLT.ECLR( RefOf (\_SB.ECLT.S1T1))))
#endif //ECLITE_PSE_SUPPORT
    }
    ...
    Method(PAT0, 1, Serialized)
    {
     ...
     \_SB.ECLT.ECLW(Local0, RefOf(\_SB.ECLT.S1A1 ))
     \_SB.ECLT.ECLC(ECLITE_WRITE_COMMAND, ECLITE_THERMAL_ALERT_THERHSOLD_UPDATE, ECLITE_S1A1_OFFSET, ECLITE_BYTES_COUNT_2)
#endif //ECLITE_PSE_SUPPORT
    }
...
#ifdef ECLITE_PSE_SUPPORT
     Store(\_SB.IETM.K10C(Arg0),Local0)
     \_SB.ECLT.ECLW(Local0, RefOf(\_SB.ECLT.S1A3 ))
     \_SB.ECLT.ECLC(ECLITE_WRITE_COMMAND, ECLITE_THERMAL_ALERT_THERHSOLD_UPDATE, ECLITE_S1A3_OFFSET, ECLITE_BYTES_COUNT_2)
#endif //ECLITE_PSE_SUPPORT
    }
...
#endif
#ifdef ECLITE_PSE_SUPPORT
      Notify(\_SB.SEN3, 0x91) // notify the participant of a trip point change event
#endif //ECLITE_PSE_SUPPORT
    }
    ...
    Method(_AC3,0,Serialized)
    {
      If(CTYP)
      {
        If (LEqual(\S3PT,0)) {Return(0xFFFFFFFF)} // Indicate DPTF driver to disable trip point
        Store(\_SB.IETM.CTOK(\S3PT),Local1) // Passive Cooling Policy
      } Else {
        If (LEqual(\S3AT,0)) {Return(0xFFFFFFFF)} // Indicate DPTF driver to disable trip point
    ...
    {
      If (LEqual(\S3CT,0)) {Return(0xFFFFFFFF)} // Indicate DPTF driver to disable trip point
    ...
    {
      If (LEqual(\S3S3,0)) {Return(0xFFFFFFFF)} // Indicate DPTF driver to disable trip point
    ...
    {
      If (LEqual(\S3HT,0)) {Return(0xFFFFFFFF)} // Indicate DPTF driver to disable trip point
